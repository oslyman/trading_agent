		...
		if (verify) {	
			...
			*/
			...
			if (content instanceof AdNetworkDailyNotification) {
				...
				if (Math.abs(receiverData.daysData[day].qualityRating - dailyNotification.getQualityScore()) > 0.01) {
					/* ERROR: Quality Rating Computation */
					System.out.println(getBasicInfoString(receiver)
							+ StringUtils.rightPad("AdNetNotification: ", 20)
							+ "ERROR: Quality Rating Computation - "
							+ "Reported Rating: " + dailyNotification.getQualityScore() + " Expected: " + receiverData.daysData[day].qualityRating
							);
				}
			...
			if (content instanceof AdNetworkReport) {
				...
				for (AdNetworkKey adNetworkKey : adNetworkReport.keys()) {
					...
					if (receiverData.daysData[cmpStartDay].cmpWon) {
						...
						if ((day - 1 <=  receiverData.daysData[cmpStartDay].cmpDayEnd) && (day - 1 >=  cmpStartDay)) {
							...
							System.out.println(getBasicInfoString(receiver)
								+ StringUtils.rightPad("AdNetworkReport: ", 20)
								+ "ERROR: reported campaign " + cmpId + " day " + (day - 1) + " out of range" +
								"["  + cmpStartDay + "," + receiverData.daysData[cmpStartDay].cmpDayEnd + "]");							
						}
					} else {
				...
				int d = day - 1;
				for(Integer campaignId : receiverData.daysData[d].cmpBudgetDailyLimits.keySet()){
				...
				if (!cmpDailyStats.isEmpty()){
					...
					System.out.println(getBasicInfoString(receiver)
							...
							+ "ERROR: impressions cost over the daily limit. Cmapaign #"+campaignId+" cost: "+cost+" limit: "+dailyLimit);
				}
				}
				}
					if ((receiverData.daysData[d].cmpWon) && (day-1 <= receiverData.daysData[d].cmpDayEnd)) {
						// for each active won campaign
						if (Math.abs(receiverData.daysData[d].cmpAdxCost - receiverData.daysData[d].adxAdnetReportedCosts) > 0.03) {
							/* ERROR: Cost Computation AdnetReport vs CmpReport */
							System.out.println(getBasicInfoString(receiver)
									+ StringUtils.rightPad("AdNetworkReport: ", 20)
									+ "ERROR: Cost Computation AdnetReport vs CmpReport - " + receiverData.daysData[d].cmpId 
									+ " Reported cmp: " + receiverData.daysData[d].cmpAdxCost + " AdNet: " + receiverData.daysData[d].adxAdnetReportedCosts
									);
						}
